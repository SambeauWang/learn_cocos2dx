...
local function createPureBreaker()
	...
	local function pathToChunk(path)
		local a = {}
		local i = 0
		...
		for word in string.gmatch(path, '[^/\\]+') do
			...
			if startRecord then
				table.insert(a,string.lower(word))
			end
			if not startRecord and word == "src" then
				print("startRecord")
				startRecord = true
			end
		end
		local chunk = table.concat(a, "/")
		return chunk
	end
	...
	local function hookfunc()
		...
		if info then
			...
			-- end
			local chunk = info.source
			local bpSet = breakpointsPerPath[chunk]
	...
	return {
		setBreakpoints = function(path, lines)
			...
			local chunk = pathToChunk(path)
			print(chunk)
			breakpointsPerPath[chunk] = t
			return verifiedLines
		end,
...
function handlers.setBreakpoints(req)
	...
	for i, bp in ipairs(req.arguments.breakpoints) do
		bpLines[i] = bp.line
		print(req.arguments.source.path,  bp.line)
	end
	local sourcepath  = string.gsub(req.arguments.source.path, "/", "\\")
